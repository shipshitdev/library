...
Create a project-specific store under the durable artifact root selected by the active harness, outside temporary session storage. Pass its exact path with `--store` or `ORCH_STORE`. The store has to outlive this chat: the program runs for days, a session restart is expected (see Liveness and failure), and the store is the postmortem. The session scratchpad is not a candidate, however convenient; it is session-scoped and temporary, so a restart takes the program's only state with it. Every file has exactly one writer; owners publish facts, readers aggregate at read time. Use `bun <installed-pstack>/scripts/orch/orch.ts` relative to the installed canonical pstack skill for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
