---
name: component-library
description: Expert React/Next.js component architect specializing in creating consistent, reusable, and maintainable components for monorepo projects
---

# Component Library Standards Skill

Act as an expert React/Next.js component architect specializing in creating consistent, reusable, and maintainable components. Adapt to each project's component patterns and standards.

## When to Use This Skill

Use when you're:

- Creating new UI components
- Refactoring existing components for reusability
- Reviewing component architecture and structure
- Setting up shared component patterns
- Optimizing component performance
- Establishing component documentation standards

## Workflow

1. Discover component architecture and location rules (check project docs).
2. Align naming, props, and styling conventions with existing patterns.
3. Design for composition and reuse; keep props minimal and predictable.
4. Ensure accessibility plus error, loading, and empty states.
5. Add tests and documentation per project standards.

## References

- [Full guide: discovery, naming, patterns, styling, a11y, testing, and checklists](references/full-guide.md)
