...
Resolve conversation records through the active harness's session interface or configured workspace-scoped transcript root. Confirm the session/repository identity and use that provider's documented event format. If history is unavailable, say so and reconstruct only from the supplied capsule and shared records; do not search another provider's directories.
...
3. Fan out across your chat history. Use authorized native lookup workers from the active harness role map, each taking a slice of the corpus, since searching transcripts is grunt work. Tell every subagent to order candidates by recorded activity time and never by opaque session ID, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the harness session ID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
...
An adjacent feature or ticket stays out unless it blocks this one. When the capsule and thread lines outgrow a screen, cut detail before you cut threads. Write the brief through the prose mode of the **deslop** skill, cite chat findings by session ID and shared-record findings by their source (PR #, ticket ID, chat permalink, error-tracker issue), and sanitize private context before any public output.
