...
**Platform note.** On Codex or another non-Claude runtime, the `comment-sicko` subagent and the Claude tool names below are Claude defaults. Resolve them via `codex-tools.md` (resolve the `pstack` skill through the active catalog).
