...
```text
/pstack repro the duplicate write first. if there's a cheap test path, /tdd it. then fix and rerun.
```
...
- **Writing a `procedure.md` freehand.** Route it through the [Authoring or modifying a skill playbook](../../../../playbooks/authoring-a-skill.md) so validation and review happen.
