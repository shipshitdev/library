...
The human enters setup by pointing Cursor at the pack's `FOR_AGENTS.md`. The bootstrap flow copies the whole pack into the target repository, then reads this file directly at `.cursor/automations/benny/skills/setup-benny/procedure.md`.
...
## 1. Copy the pack and resolve canonical shared skills
...
Make the canonical Shipshit dependencies available in the target harness using
its supported project-scoped installation mechanism. Preserve the existing role
configuration and unrelated registrations. Never enable the original Pstack
plugin or copy an upstream plugin cache into the repository.

Start a fresh agent rooted in the target repository. Verify canonical `how`,
`why`, `tdd`, `deslop` prose mode and `pstack` resolve through its active
catalog. Load the required principle resources from the installed pstack directory.
If project-scoped discovery cannot expose these dependencies, report the missing
capability and leave the automations dormant.

The Benny files are read directly from `.cursor/automations/benny/`. Do not add that directory to a plugin manifest or expect its `procedure.md` files to appear in the slash-skill list.

Tell the user that the approved project skill registration, `.cursor/automations/benny/`, and any referenced secret-free configuration must be committed before either automation is enabled. Do not commit them unless the user asks.
...
- Name `benny-triage`.
- Read and follow `.cursor/automations/benny/skills/triage-issue-reports/procedure.md` for every run.
...
- Read and follow `.cursor/automations/benny/skills/reproduce-and-fix-issues/procedure.md` for every run.
...
- Direct instruction to read `.cursor/automations/benny/skills/triage-issue-reports/procedure.md`
...
- Name and description
- Direct instruction to read `.cursor/automations/benny/skills/reproduce-and-fix-issues/procedure.md`
