...
These skills share one tree across Claude Code and Codex. Read [`references/provider-dispatch.md`](provider-dispatch.md) whenever a configured role launches. It defines the provider-qualified model descriptors, native/external route table, launcher, isolation, receipts, and dropout policy. Children never choose routes. When a skill names a Claude tool or built-in skill (`run`, `verify`, `skill-creator`), read [`references/codex-tools.md`](codex-tools.md) for the Codex equivalent.
...
- Any prose surface → the prose mode of the **deslop** skill. Your reply is a prose surface; write it per **Writing the reply**. Agent-facing prose also follows the **skill-creator** skill (Claude Code's authoring guidance for SKILL.md files).
...
Read the corresponding `references/principle-*.md` resource in full for any principle you apply. Principle labels below are resources, not separate catalog skills. Each entry names when it applies.
...
**Just do it.** Use the capabilities authorized by the active harness. Proceed within the caller’s authorized actions; external messages require explicit authorization.

**Require action-specific authorization** for irreversible writes: force-push to shared branches, deploys, data deletion and customer messages. Existing authorization satisfies this gate only for the same actions and scope.
