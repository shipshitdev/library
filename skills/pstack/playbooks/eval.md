...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's session record through the active harness interface or the exact transcript path returned by that candidate's launch. Confirm task and workspace identity. Use the provider's documented event schema; if tool-read evidence is unavailable, report chain-following as unverified rather than searching another provider's history. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
