...
import { Model } from "mongoose";{Entity}, {Entity}Document } from "./schemas/{{entity}}.schema";{Entity}Dto } from "./dto/create-{{entity}}.dto";
...
export class {{Entity}}sService {
  ...
  async findOne(id: string, userId: string): Promise<{Entity}> {
    const _query = this.entityModel.findOne({ _id: id, userId });
  ...
  async update(
    ...
    const _query = this.entityModel.findOneAndUpdate(
      { _id: id, userId },
      update{{Entity}}Dto,
      { new: true },
    );
  ...
  async remove(id: string, userId: string): Promise<void> {
    const query = this.entityModel.deleteOne({ _id: id, userId });
