...
**Platform note.** On Codex or another non-Claude runtime, the parallel per-feature source readers below map to `spawn_agent` fan-out, and the project-local skill lives under your runtime's skills location, not a hardcoded provider directory. See `codex-tools.md` (resolve the `pstack` skill through the active catalog).
