...
Bind to localhost by default. If the user authorized access from Tailscale peers, select the approved network interface and port and verify access controls before exposing it. A wildcard bind also exposes other interfaces and requires explicit authorization.
