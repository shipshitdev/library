...
This skill enables Cursor to discover email addresses associated with a domain using a hybrid approach. Cursor will use free methods first (web scraping, pattern guessing, WHOIS lookup) and can optionally use APIs (Hunter.io, Apollo.io, etc.) for enhanced results when API keys are available.

## When to Use This Skill

This skill activates automatically when users:

- Need to scan a domain and find associated email addresses
- Want to find contact emails for a company
- Need to discover email addresses to replace email hunter functionality
- Want to find email patterns for a domain
- Need to verify email addresses found through other means
- Want to enrich contact data with discovered emails

## Project Context Discovery

**Before finding emails, discover the project's context:**

1. **Scan Project Documentation:**
   - Check for existing email discovery tools or processes
   - Review API keys available (Hunter.io, Apollo.io, etc.)
   - Look for data schemas or structures for email storage
   - Check for compliance or privacy requirements

2. **Identify Requirements:**
   - Determine if API keys are available or if free methods only
   - Check for rate limiting requirements
   - Review legal/compliance constraints
   - Identify validation requirements

3. **Use Project-Specific Skills:**
   - Check for `[project]-email-finder` skill
   - Review project-specific workflows
   - Follow project's data standards
...
Here's a complete example that uses the hybrid approach:
...
async function findEmailsForDomain(domain: string): Promise<EmailResult[]> {
  ...
  try {
    // WHOIS lookup (if Python script available)
    // This would need to be called via subprocess or separate service
  } catch (error) {
...
Always verify email addresses before using them, especially if they were guessed or scraped.
...
Use Hunter.io or other services for more reliable verification:

```typescript
// See Hunter.io verification example above
```
...
Always implement rate limiting to avoid overwhelming servers or hitting API limits:
...
**Example 1: "Scan example.com and find all email addresses"**

- Use web scraping to check common pages
- Check WHOIS data
- If API keys available, use Hunter.io or Apollo.io
- Return deduplicated list of emails with sources

**Example 2: "Find emails for contact@example.com domain"**

- Focus on contact page first
- Use pattern guessing for common contact patterns
- Verify emails found

**Example 3: "Replace email hunter - find emails for acme.com"**

- Use hybrid approach (free methods + APIs if available)
- Provide results in similar format to email hunter
