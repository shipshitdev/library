...
✅ **Skills:** All platforms have 42 skills - complete parity achieved

- Claude: 42 skills
- Codex: 42 skills
- Cursor: 42 skills

✅ **Commands:** Commands are primarily Cursor-specific

- Claude: 1 command (start.md)
- Codex: 0 commands
- Cursor: 24 commands

✅ **Ready to Use:** All skills are independent, have correct platform paths, and are ready for use
...
If you've renamed this repository from "skills" to "library", you'll need to update your symlinks to point to the new path.

### Using the Migration Script

A migration script is provided to automatically update your symlinks:

```bash
# First, test what would be changed (dry run)
./migrate-symlinks.sh --dry-run
...
- Automatically detect your existing symlinks pointing to the old "skills" path
- Update them to point to the new "library" path
- Handle all standard symlinks: `~/.claude/skills`, `~/.codex/skills`, `~/.cursor/commands`, `~/.cursor/skills`

**Options:**

- `--dry-run`: Preview changes without applying them
- `--old-path PATH`: Manually specify the old repository path (if auto-detection fails)
- `--new-path PATH`: Manually specify the new repository path (defaults to replacing "skills" with "library")

### Manual Migration

If you prefer to update symlinks manually:

1. Remove the old symlinks:

   ```bash
   rm ~/.claude/skills ~/.codex/skills ~/.cursor/commands ~/.cursor/skills
   ```

2. Create new symlinks pointing to the "library" path:
...
3. Verify the symlinks are working:

   ```bash
   ls -la ~/.claude/skills ~/.codex/skills ~/.cursor/commands ~/.cursor/skills
   ```

**Note:** Any project repositories that symlink to specific skills from this repo will also need their symlinks updated to use the new "library" path.
...
|---------|-------------|-------|--------|--------|
| **analyze-codebase** | Generate comprehensive analysis of codebase structure, architecture, and organization. | N/A | N/A | [`.cursor/commands/analyze-codebase.md`](.cursor/commands/analyze-codebase.md) |
| **api-test** | Generate, run, and validate API tests for NestJS endpoints covering authentication, authorization, validation, error handling, and edge cases. | N/A | N/A | [`.cursor/commands/api-test.md`](.cursor/commands/api-test.md) |
| **bug** | Quick bug capture and documentation workflow for tracking issues and creating bug reports. | N/A | N/A | [`.cursor/commands/bug.md`](.cursor/commands/bug.md) |
| **clean** | Unified cleanup workflow for tasks and session files to maintain organized project structure. | N/A | N/A | [`.cursor/commands/clean.md`](.cursor/commands/clean.md) |
| **code-review** | Comprehensive code review workflow with security, performance, and best practices analysis. | N/A | N/A | [`.cursor/commands/code-review.md`](.cursor/commands/code-review.md) |
| **deploy** | Streamline deployment workflows for React, Next.js, NestJS applications to various environments (AWS, Vercel, etc.). | N/A | N/A | [`.cursor/commands/deploy.md`](.cursor/commands/deploy.md) |
| **docs-generate** | Generate comprehensive documentation from codebase including API docs, architecture diagrams, and guides. | N/A | N/A | [`.cursor/commands/docs-generate.md`](.cursor/commands/docs-generate.md) |
| **docs-update** | Update existing documentation to reflect codebase changes and maintain documentation accuracy. | N/A | N/A | [`.cursor/commands/docs-update.md`](.cursor/commands/docs-update.md) |
| **end** | Document session work and clear context before ending a development session. | N/A | N/A | [`.cursor/commands/end.md`](.cursor/commands/end.md) |
| **inbox** | Process inbox items and organize tasks from the project inbox into actionable items. | N/A | N/A | [`.cursor/commands/inbox.md`](.cursor/commands/inbox.md) |
| **migrate** | Manage MongoDB (and other database) migrations safely with version control and rollback capabilities. | N/A | N/A | [`.cursor/commands/migrate.md`](.cursor/commands/migrate.md) |
| **new-cmd** | Create new custom slash commands following established patterns and best practices. | N/A | N/A | [`.cursor/commands/new-cmd.md`](.cursor/commands/new-cmd.md) |
| **new-session** | Create new session files from templates for tracking development work and decisions. | N/A | N/A | [`.cursor/commands/new-session.md`](.cursor/commands/new-session.md) |
| **optimize-prompt** | Optimize AI prompts for better results, clarity, and effectiveness in agent interactions. | N/A | N/A | [`.cursor/commands/optimize-prompt.md`](.cursor/commands/optimize-prompt.md) |
| **performance** | Analyze and optimize performance for React, Next.js, NestJS applications covering frontend, backend, database, and infrastructure. | N/A | N/A | [`.cursor/commands/performance.md`](.cursor/commands/performance.md) |
| **quick-fix** | Daily task list management and quick fixes workflow for handling urgent issues. | N/A | N/A | [`.cursor/commands/quick-fix.md`](.cursor/commands/quick-fix.md) |
| **refactor-code** | Refactoring workflows for improving code quality, structure, and maintainability. | N/A | N/A | [`.cursor/commands/refactor-code.md`](.cursor/commands/refactor-code.md) |
| **review-pr** | Pull request review checklist and workflow for code quality and standards compliance. | N/A | N/A | [`.cursor/commands/review-pr.md`](.cursor/commands/review-pr.md) |
| **scaffold** | Unified project scaffolder for creating new projects or adding components to existing ones. | N/A | N/A | [`.cursor/commands/scaffold.md`](.cursor/commands/scaffold.md) |
| **security-audit** | Comprehensive security audit for React, Next.js, NestJS applications covering dependencies, code patterns, configuration, and best practices. | N/A | N/A | [`.cursor/commands/security-audit.md`](.cursor/commands/security-audit.md) |
| **start** | Bootstrap session context and initialize development environment for productive work sessions. | N/A | [`.claude/commands/start.md`](.claude/commands/start.md) | [`.cursor/commands/start.md`](.cursor/commands/start.md) |
| **task** | Create and manage tasks with PRDs, tracking, and documentation workflows. | N/A | N/A | [`.cursor/commands/task.md`](.cursor/commands/task.md) |
| **test** | Test tracking and management workflow for organizing and running test suites. | N/A | N/A | [`.cursor/commands/test.md`](.cursor/commands/test.md) |
| **validate** | Unified validation workflow for documentation, sessions, and tasks to ensure quality and completeness. | N/A | N/A | [`.cursor/commands/validate.md`](.cursor/commands/validate.md) |
