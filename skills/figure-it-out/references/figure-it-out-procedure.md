...
Open a todolist whose first item is to read the Principles section of the **pstack** skill. Then add the phases below as todos.
...
- For one-way-door design decisions, run the **architect** skill (it runs **arena**) with diverse, isolated, opinionated candidates and an independent read-only judge selected by the authorized harness role map. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
