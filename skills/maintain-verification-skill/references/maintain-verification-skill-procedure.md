...
**Platform note.** On Codex or another non-Claude runtime, the parallel per-feature source readers below use the harness's available native delegation interface, and the project-local skill lives under your runtime's skills location, not a hardcoded provider directory. See `codex-tools.md` (resolve the `pstack` skill through the active catalog).
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (discover candidates through the active harness’s configured project skill root). Several candidates → ask which one; none → stop and point at `/create-verification-skill` instead of inventing a target.
