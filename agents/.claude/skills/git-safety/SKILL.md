...
- Archive services (archive.org, GitFront, etc.) may have captured snapshots
- Forks retain the original history
- CI/CD logs may contain the values
- Local clones on other machines still have the data

**ALWAYS rotate leaked credentials immediately.** This skill helps clean history, but credential rotation is the ONLY way to secure a leaked secret.

## Modes of Operation

### 1. `/git-safety scan` - Detect Sensitive Files

Scan the repository for sensitive files in current state and git history.

### 2. `/git-safety clean` - Remove from History

Remove sensitive files from git history using git-filter-repo or BFG.

### 3. `/git-safety prevent` - Set Up Prevention

Configure .gitignore and pre-commit hooks to prevent future leaks.
...
### Scan
```bash
# Quick scan for sensitive files in history
git log --all --pretty=format: --name-only --diff-filter=A | sort -u | grep -iE 'env|secret|credential|key|token|password'
```
...
1. **Ask which mode they want**: scan, clean, prevent, or full audit
2. **For scan**: Run the detection commands and provide formatted report
3. **For clean**:
   - Confirm they understand history rewriting implications
   - Verify backup exists
   - Ask which specific files to remove
   - Provide exact commands (do NOT auto-execute force push)
4. **For prevent**: Create/update .gitignore and offer to set up pre-commit hook
5. **Always emphasize**: Credential rotation is mandatory for any leaked secret
