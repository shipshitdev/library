...
**Checking for upstream changes:** when upstream has moved ahead of the synced marker above, diff [`pstack/skills/interrogate/SKILL.md`](https://github.com/cursor/plugins/blob/main/pstack/skills/interrogate/SKILL.md) on `main` since commit `bdf7aa355337`, port anything worth bringing home, then bump `metadata.upstream_commit` (or `metadata.upstream_version`) and `metadata.last_synced` in `SKILL.md` and this table.
